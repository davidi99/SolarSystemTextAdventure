...
uses
  ...
  FMX.Objects, FMX.Effects, FMX.Ani, System.Generics.Collections, FMX.Memo.Types;
...
var
  MemoBackground: TRectangle;
begin
  // Make memo background compatible with dark theme
  // The RectStoryBg behind it provides the dark background
  
  // Set button background colors
  BtnOption1.TintColor := $FF1A3A5A;  // Dark blue
  BtnOption2.TintColor := $FF1A3A5A;
  BtnOption3.TintColor := $FF1A3A5A;
  BtnOption4.TintColor := $FF1A3A5A;
  
  // Ensure memo is ready
  MemoStory.Lines.BeginUpdate;
  try
    MemoStory.Lines.Clear;
    MemoStory.Lines.Add('=== SOLAR SYSTEM EXPLORER ===');
    MemoStory.Lines.Add('');
    MemoStory.Lines.Add('Starting initialization...');
  finally
    MemoStory.Lines.EndUpdate;
  end;
  ...
  // Debug output
  MemoStory.Lines.Add('Creating location database...');
  Application.ProcessMessages;
  
  InitializeLocations;
  
  // Debug output
  MemoStory.Lines.Add(Format('SUCCESS: Loaded %d locations', [FLocations.Count]));
  MemoStory.Lines.Add('');
  MemoStory.Lines.Add('Navigating to Earth Orbit...');
  Application.ProcessMessages;
